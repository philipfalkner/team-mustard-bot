...
async function startMinecraft (logger, message) {
  ...
  const functionStart = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then(response => response.json())
  logger.debug(functionStart)

  logger.info('Waiting for Minecraft server to start...')
  while (moment().isBefore(startupTimeLimit)) {
    const functionStatus = await fetch(functionStart.statusQueryGetUri, { headers: { 'Content-Type': 'application/json' } }).then(response => response.json())
